var myModalEl, kanbanboard, scroll, addNewBoard, addMember,
  profileField, reader, tasks_list = [document.getElementById(
      "kanbanboard")];
      
    jQuery(".task-list").each(function() {
      tasks_list.push(document.getElementById( $(this).attr('id')));
    });

function noTaskImage() {
  Array.from(document.querySelectorAll("#kanbanboard .tasks-list"))
    .forEach(function (e) {
      0 < e.querySelectorAll(".tasks-box")
        .length ? e.querySelector(".tasks")
        .classList.remove("noTask") : e.querySelector(".tasks")
        .classList.add("noTask")
    })
}
...
function  update(){
 ...
 let url = $("#kanpan_update").val();
     $(".tasks-list").each(function(){
    let task = [];
    s = $(this).data('status')
  
    $(this).find('.tasks-box').map(function(){
      task.push($(this).data('task'))
    })
    status[s] = task
  });
  // console.log(status)
...
function taskCounter() {
  (task_lists = document.querySelectorAll(
  "#kanbanboard .tasks-list")) && Array.from(task_lists)
    .forEach(function (e) {
      ...
      Array.from(tasks)
        .forEach(function (e) {
         ...
         badge = e.querySelector(".totaltask-badge")
        .innerText = "",
         badge = e.querySelector(".totaltask-badge")
        .innerText = task_counted
    })
...
//     , t = "review_task_" + a;
//   kanbanlisthtml = '<div class="tasks-list" id=' + ("remove_item_" +
//       a) +
...
//     .insertAdjacentHTML("beforeend", kanbanlisthtml), document
//     .getElementById("addBoardBtn-close")
//     .click(), noTaskImage(), taskCounter(), drake.destroy(), 
//     tasks_list.push(document.getElementById(t)), drake = dragula(
//       tasks_list)
//     .on("out", function (e, a) {
...
//   var e = document.getElementById("firstnameInput")
//     .value
...
//     e + '">' + a + "</a>", document.getElementById("newMembar")
//     .insertAdjacentHTML("afterbegin", newMembar), document
//     .getElementById("btn-close-member")
//     .click()
// }

tasks_list && ((myModalEl = document.getElementById(
    "deleteRecordModal")) && myModalEl.addEventListener(
    "show.bs.modal"
    , function (e) {
      document.getElementById("delete-record")
        .addEventListener("click", function () {
          e.relatedTarget.closest(".tasks-box")
            .remove(), document.getElementById("delete-btn-close")
            .click(), taskCounter() 
        })
  ...
  document.getElementById("addNewBoard")
  .addEventListener("click", newKanbanbaord), (addMember = document
    .getElementById("addMember")) && (document.getElementById(
      "addMember")
    .addEventListener("click", newMemberAdd), profileField =
    document.getElementById("profileimgInput"), reader =
    new FileReader, profileField.addEventListener("change",
      function (e) {
        reader.readAsDataURL(profileField.files[0]), reader.onload =
          function () {
            var e = reader.result;
